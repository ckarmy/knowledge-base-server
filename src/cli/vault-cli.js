...
export async function vaultReindex() {
  ...
  // Reindexing should keep keyword and semantic content in sync by default.
  // --embeddings remains accepted; --text-only is the explicit offline escape hatch.
  const withEmbeddings = !process.argv.includes('--text-only');
